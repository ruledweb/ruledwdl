...
> **Maintained in**: [`specifications/registry/v2.0.md`](file:///home/pradeep/cloudflare/workers/wdl-core/specifications/registry/v2.0.md)

---

## 1. Overview

The **`REGISTRY v2.0`** specification elevates REGISTRY into a type-safe, inheritable design system. It adds global theme tokens (`__tokens__`), unbracketed variable placeholders (`prefix-$_{scoped-var}`), token inheritance (`uses`), responsive breakpoints, container queries, and component variant maps.
...
2. **Variable References & Placeholders**:
   ...
   * `prefix-$_{scoped-var}`: Unbracketed placeholder syntax (e.g. `p-$_{pad}`). Auto-expands to `p-[var(--spacing-card)]` or `p-[1.5rem]`.
...
* **`2.0`** (Core v0.2.0): Introduced structured design system (`__tokens__`, `prefix-$_{scoped-var}`, `uses` inheritance, variants, states, breakpoints, containers).
