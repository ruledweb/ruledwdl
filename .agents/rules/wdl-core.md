...
1. **Verification**: Run `npm test` across all packages to ensure smoke, event, DOM, and nested tests pass cleanly.
...
7. **Build Nested Package**: Run `npm run build --prefix packages/nested` to compile `@ruledwdl/nested`.
8. **Version Bump**: Bump versions in root `package.json` and `packages/*/package.json`.
9. **Git Push & Tags**: Push commit and tags to the remote repository using `ruledweb` SSH access:
   ```bash
